...
def input_main_query():
    st.session_state["main_query_input"] = st.text_input("Enter your main query here:")
...
def input_data_specs():
    ...
    for i in range(st.session_state['num_rows']):
        ...
        with col1:
            st.text_input(f"Column 1, Row {i+1}", key=f"col1_{i}")
        with col2:
            st.text_input(f"Column 2, Row {i+1}", key=f"col2_{i}")
