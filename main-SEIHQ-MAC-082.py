...
def extract_policy_doc_info(main_query, text_embeddings, text_chunks, col_embeddings, openai_apikey):
    ...
    for col_name in col_embeddings:
        ...
        ## resp fmt: {{'variable_name': {col_nm}, 'list_of_quotes': [{{'quote': '...', 'page_number': '...', 'explanation': '...'}}, ...]}}"
        policy_doc_data[col_name] = resp["list_of_quotes"]
...
def main(gpt_analyzer, openai_apikey):
    ...
    for pdf in gpt_analyzer.pdfs:
        ...
        try:
            ...
            policy_info = extract_policy_doc_info(gpt_analyzer.main_query, pdf_embeddings, pdf_text_chunks, var_embeddings, openai_apikey)
            # 4) Output Results
